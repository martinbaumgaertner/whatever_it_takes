...
set.seed(42)
split <- sample(1:nrow(dfm), size = nrow(dfm) * 0.8, replace = F)
dfm_train <- dfm[split, ]
dfm_test <- dfm[-split, ]
rm(dfm)
...
INTEREST_RATE <- fredr(c("IR3TIB01USM156N")) %>% select(date, INTEREST_RATE = value) # 3-Month Interbank Interest Rate (IR3TIB01USM156N)
...
function_document_matrix <-
  function(dfm, embedding, macro_df) {
    # shared vocab
    dfm_vocab <- dfm %>%
      featnames() %>%
      tibble() %>%
...
train <- train %>%
  ...
  mutate(date = as.Date(date1$quarter)) %>%
  left_join(macro_df)
train <- train %>% mutate(SENTIMENT = (negative - positive) / (negative + positive), HD = (HAWK - DOVE) / (HAWK + DOVE), SENTIMENT2 = (NEGATIVE - POSITIVE) / (NEGATIVE + POSITIVE)) # 24 dimensional representation!
train <- train %>%
...
test <- test %>%
  convert(to = "data.frame") %>%
  tibble() %>%
  mutate(SENTIMENT = (negative - positive) / (negative + positive), HD = (HAWK - DOVE) / (HAWK + DOVE), SENTIMENT2 = (NEGATIVE - POSITIVE) / (NEGATIVE + POSITIVE)) # 24 dimensional representation!
test <- test %>%
  mutate(date = as.Date(date1$quarter)) %>%
  left_join(macro_df)
test <- test %>%
